/* global baysideCommunityBgColors, baysideCommunityColor, jQuery, wp, _ */
/**
...
( function() {
	...
	wp.customize.bind( 'ready', function() {
		...
		wp.customize( 'accent_hue', function( value ) {
			value.bind( function( to ) {
				...
				Object.keys( baysideCommunityBgColors ).forEach( function( context ) {
					var backgroundColorValue;
					if ( baysideCommunityBgColors[ context ].color ) {
						backgroundColorValue = baysideCommunityBgColors[ context ].color;
					} else {
						backgroundColorValue = wp.customize( baysideCommunityBgColors[ context ].setting ).get();
					}
					baysideCommunitySetAccessibleColorsValue( context, backgroundColorValue, to );
				} );
			} );
		} );
		...
		Object.keys( baysideCommunityBgColors ).forEach( function( context ) {
			wp.customize( baysideCommunityBgColors[ context ].setting, function( value ) {
				value.bind( function( to ) {
					// Update the value for our accessible colors for this area.
					baysideCommunitySetAccessibleColorsValue( context, to, wp.customize( 'accent_hue' ).get(), to );
				} );
			} );
		} );

		// Show or hide retina_logo setting on the first load.
		baysideCommunitySetRetineLogoVisibility( !! wp.customize( 'custom_logo' )() );

		// Add a listener for custom_logo changes.
		wp.customize( 'custom_logo', function( value ) {
			value.bind( function( to ) {
				// Show or hide retina_logo setting on changing custom_logo.
				baysideCommunitySetRetineLogoVisibility( !! to );
			} );
		} );
	} );
	 ...
	 * @return {void}
	 */
	function baysideCommunitySetAccessibleColorsValue( context, backgroundColor, accentHue ) {
		var value, colors;
		...
		colors = baysideCommunityColor( backgroundColor, accentHue );
	 ...
	 *
	 * @since Bayside Community 1.3
	 *
	 * @param {boolean} visible The visible value.
	 *
	 * @return {void}
	 */
	function baysideCommunitySetRetineLogoVisibility( visible ) {
