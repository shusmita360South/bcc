/* global baysideCommunityBgColors, baysideCommunityPreviewEls, jQuery, _, wp */
/**
...
( function( $, api, _ ) {
		...
		value.bind( function() {
			...
			setTimeout( function() {
				Object.keys( baysideCommunityBgColors ).forEach( function( context ) {
					baysideCommunityGenerateColorA11yPreviewStyles( context );
				} );
			}, 50 );
		} );
	} );
	 ...
	 * @return {void}
	 */
	function baysideCommunityGenerateColorA11yPreviewStyles( context ) {
		// Get the accessible colors option.
		var a11yColors = window.parent.wp.customize( 'accent_accessible_colors' ).get(),
			stylesheedID = 'baysidecommunity-customizer-styles-' + context,
			stylesheet = $( '#' + stylesheedID ),
			styles = '';
		// If the stylesheet doesn't exist, create it and append it to <head>.
		if ( ! stylesheet.length ) {
			$( '#baysidecommunity-style-inline-css' ).after( '<style id="' + stylesheedID + '"></style>' );
			stylesheet = $( '#' + stylesheedID );
		}
		if ( ! _.isUndefined( a11yColors[ context ] ) ) {
			// Check if we have elements defined.
			if ( baysideCommunityPreviewEls[ context ] ) {
				_.each( baysideCommunityPreviewEls[ context ], function( items, setting ) {
	...
	$( document ).ready( function() {
		baysideCommunityGenerateColorA11yPreviewStyles( 'content' );
		baysideCommunityGenerateColorA11yPreviewStyles( 'header-footer' );
	} );
