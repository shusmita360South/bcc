( function() {
	 ...
	 * @since  1.0.0
	 */
	butterbean.views.register_control( 'multilocation', {

		// Adds custom events.
		events : {
			'click .js-butterbean-multilocation-handlediv' : 'multilocationClose',

			'click .js-butterbean-multilocation-add'    : 'multilocationAdd',
			'click .js-butterbean-multilocation-remove' : 'multilocationRemove',

			'change .js-butterbean-multilocation-title' : 'multilocationTitleUpdate',
		...
		ready: function() {
			...
			$( '.butterbean-multilocations' ).sortable({
				opacity: 0.6,
				revert: 200,
				cursor: 'move',
				handle: '.butterbean-multilocation-sortable-handle',
				stop: function( e, ui ) {
					_this.multilocationUpdateOrder();
				}
			});
		},

		multilocationClose: function( e ) {
			e.preventDefault();

			var $ = jQuery,
				$target = $( e.currentTarget ),
				$multilocation = $target.parents( '.butterbean-multilocation' );

			$multilocation.toggleClass( 'closed' );
		},

		multilocationAdd: function( e ) {
			e.preventDefault();

			var $ = jQuery,
				$multilocationTemplate = $( '.butterbean-multilocation-clonable' ),
				index = $( '.butterbean-multilocation' ).last().index() + 1, // items are 0 indexed
				multilocationName = 'Slide ' + (index + 1),
				settingName = this.model.attributes.field_name,
				$multilocationsContainer = $( '.butterbean-multilocations' );

			cloneHtml = $multilocationTemplate.prop( 'outerHTML' );

			cloneHtml = cloneHtml.replace( /_multilocation_name_/g, multilocationName );
			cloneHtml = cloneHtml.replace( /_setting_name_/g, settingName );
			cloneHtml = cloneHtml.replace( /_index_/g, index );

			$clone = $( cloneHtml ).appendTo( $multilocationsContainer ).removeClass( 'butterbean-multilocation-clonable' );
		},

		multilocationRemove: function( e ) {
			e.preventDefault();
			...
			$multilocation.remove();
		},

		multilocationTitleUpdate: function( e ) {
			var $ = jQuery,
				$target = $( e.currentTarget ),
				$multilocation = $target.parents( '.butterbean-multilocation' ),
				$headingTitle = $multilocation.find( 'h2 .-multilocation-title' );

				$headingTitle.text( $target.val() );
		},

		multilocationUpdateOrder: function() {
			var $ = jQuery;

			$( '.butterbean-multilocations .butterbean-multilocation' ).each( function( i ) {
				var indexText = i + 1;
		...
		showmodal : function( e ) {
			...
			var $ = jQuery,
				$target = $( e.currentTarget ),
				$imageIdInput = $target.parents( '.butterbean-multilocation' ).find( '.butterbean-img-id-input' ),
				$image = $target.parents( '.butterbean-multilocation' ).find( '.butterbean-img' );
