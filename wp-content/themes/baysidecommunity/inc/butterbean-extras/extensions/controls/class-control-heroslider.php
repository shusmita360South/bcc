<?php
/**
 * Heroheroslider control class.
 */
class ButterBean_Control_Heroslider extends ButterBean_Control {
	...
	public function __construct( $manager, $name, $args = array() ) {
		...
		$this->l10n = wp_parse_args(
			...
			array(
				...
				'heroslide_color'         => esc_html__( 'Slide Color',         'butterbean' ),
				'remove_heroslide'         => esc_html__( 'Remove Slide',         'butterbean' ),
				'add_heroslide'            => esc_html__( 'Add Slide',            'butterbean' )
			)
		);
	}
	...
	public function enqueue() {
		wp_enqueue_script( 'butterbean-heroslider', get_template_directory_uri() . '/inc/butterbean-extras/extensions/js/butterbean-heroslider.js', array( 'butterbean' ), 2, true );
		wp_enqueue_style( 'butterbean-heroslider', get_template_directory_uri() . '/inc/butterbean-extras/extensions/css/butterbean-heroslider.css', array(), 2 );
	}
...
function butterbean_sanitize_heroslider( $heroslider_data ) {
	if ( ! $heroslider_data || ! is_array( $heroslider_data ) ) {
		$heroslider_data = array();

		return $heroslider_data;
	}

	usort( $heroslider_data, function ( $item1, $item2 ) {
